...
if [ $WANLAN_ENABLE = "checked" ]; then
	...
	uci set network.lan.ifname='eth1'  # Note eth1 LAN port is particular for 841 device
fi
...
if [ $WANPORT = "Ethernet" ]; then
	...
	uci set network.wan.ifname='eth0'   # Note eth0 WAN port is particular for 841 device
...
if [ $WANPORT = "Mesh" ]; then
	...
	if [ $WANLAN_ENABLE = "checked" ]; then
		uci set network.lan.ifname='eth1 eth0' # Note eth1 LAN port is particular for 841 device
		uci set network.wan.ifname='bat0'
	else
		uci set network.lan.ifname='eth1'      # Note eth1 LAN port is particular for 841 device
		uci set network.wan.ifname='bat0 eth0' # Note eth0 WAN port is particular for 841 device
	fi
