...
SETSHA="TRUE" 
SHA="48461b5abc382b26edd0d2637f0f7b"    # lede-17.01.1 as at 18/4/2017


USAGE1="Usage:   ./Setup-LEDE-17_01_1.sh   /your-preferred-source-installation-path "
USAGE2="Example: ./Setup-LEDE-17_01_1.sh   ~/LEDE/my-new-build-env"
...
./scripts/feeds install -a -p targets         >> feeds-install.log
...
printf "###########\n LuCi\n" >> gitlog.txt
SHA_LUCI=`git -C ./feeds/luci   log -n 1 --abbrev-commit| grep -A 3 commit | tee -a gitlog.txt | grep commit | cut -d " " -f 2`
...
src-git targets https://github.com/openwrt/targets.git
src-git luci https://git.lede-project.org/project/luci.git^$SHA_LUCI
